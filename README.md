...
| 服务 | 职能 | 地址 | 进度 | 备注 |
|---|---|---|---|---|
| GitLab | 工程资产（代码/文档/配置/MR/Release 声明） | http://192.168.199.131:8080 · ssh://git@192.168.199.131:2222 | 🟢 已就绪 | 19.1.3-ce，数据 `/dockerData/gitlab` |
| Plane | 项目工作（需求/任务/进度/里程碑） | http://192.168.199.131:3000 | 🟢 已就绪 | 1.4.0，workspace=`kdev` |
| Jenkins | 执行层（Build/Test/Package/Deploy） | http://192.168.199.131:8082 | 🟢 已就绪 | 2.568.2-jdk21，2 job 全链路打通 + Gitea webhook 自动触发 + win-build 节点在线 |
| Nexus | 制品（包/镜像/依赖/Registry） | http://192.168.199.131:8081（UI）· http://192.168.199.131:8083（Registry） | 🟢 已就绪 | 数据 `/dockerData/nexus` |
| RagFlow | 知识索引（检索/RAG，可重建投影） | http://192.168.199.131:9380 | 🟡 已搭建·待接入数据 | 0.26.4，模型对接完成 |
| AI Agent | 横向智能入口（不持有数据/不绕过权限） | — | ⚪ 规划中 | MCP 接入各组件 |

> 进度图例：🟢 已就绪（链路可用）· 🟡 已搭建·待接入 · ⚪ 规划中
